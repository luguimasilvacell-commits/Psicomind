-- Migration: Refazer todos os vínculos e agendamentos
-- Data: 2025-01-09
-- Descrição: Limpa e recria dados de agendamentos com vínculos corretos e dados robustos

-- 1. Limpar dados existentes (mantendo estrutura)
DELETE FROM prontuarios WHERE agendamento_id IS NOT NULL;
DELETE FROM transacoes_financeiras WHERE agendamento_id IS NOT NULL;
...
-- Pacientes para o psicólogo de teste
('11111111-1111-1111-1111-111111111111', '975cdada-b05b-4604-b4ab-e664aad693eb', 'Ana Silva Costa', '123.456.789-01', '(11) 99999-1111', 'ana.silva@email.com', '1990-03-15', '{"rua": "Rua das Flores, 123", "bairro": "Centro"}', '01234-567', 'São Paulo', 'SP', 'Professora', 'solteira', '(11) 88888-1111', 'Paciente com ansiedade generalizada', 'ativo'),
...
('33333333-3333-3333-3333-333333333333', '975cdada-b05b-4604-b4ab-e664aad693eb', 'Maria Santos Lima', '456.789.123-03', '(11) 99999-3333', 'maria.santos@email.com', '1992-11-08', '{"rua": "Rua Augusta, 789", "bairro": "Consolação"}', '01305-000', 'São Paulo', 'SP', 'Designer', 'divorciada', '(11) 88888-3333', 'Paciente com síndrome do pânico', 'ativo'),
...
('55555555-5555-5555-5555-555555555555', '975cdada-b05b-4604-b4ab-e664aad693eb', 'Fernanda Costa Silva', '159.753.486-05', '(11) 99999-5555', 'fernanda.costa@email.com', '1995-04-18', '{"rua": "Rua Haddock Lobo, 654", "bairro": "Cerqueira César"}', '01414-001', 'São Paulo', 'SP', 'Psicóloga', 'solteira', '(11) 88888-5555', 'Paciente com burnout', 'ativo')
ON CONFLICT (id) DO NOTHING;

-- 3. Criar agendamentos distribuídos ao longo de várias datas com diferentes status
...
-- Agendamentos históricos (semana passada) com diferentes status
...
-- Agendamentos para o final do mês (2025-01-30, 2025-01-31)
...
-- 4. Criar algumas transações financeiras vinculadas aos agendamentos realizados
...
-- 5. Criar índices para otimizar performance
...
-- Comentários finais
COMMENT ON TABLE agendamentos IS 'Tabela de agendamentos com vínculos corretos e dados robustos - Atualizada em 2025-01-09';
COMMENT ON TABLE pacientes IS 'Tabela de pacientes com dados completos - Atualizada em 2025-01-09';